...
## الترحيل من البوّابة القديمة

قاعدة `bawsalati` منفصلة تماماً، وقاعدة `portal` القديمة تبقى دون مساس.
`npm run migrate:legacy` يقرأ منها فقط، وينقل:

- البرامج الدراسية كاملة (141).
- موارد المكتبة كمسودّات غير منشورة، لأن روابطها في النظام القديم عناصر نائبة.

ولا ينقل: نتائج الاختبارات القديمة (بيانات تجريبية بأسماء غير حقيقية)، والاستشارات القديمة
(نصوصها العربية تلفت في عنقود بترميز WIN1252)، وبيانات المنصات الأخرى.
