...
const INTRO = [
  ...
  "النتيجة مؤشّر يساعدك ويساعد أخصائي التوجيه المهني على اتخاذ قرار أفضل، ويعتمدها الأخصائي بعد مراجعتها معك.",
].join("\n\n");
...
async function seedUsers() {
  ...
  console.log(
    process.env.NODE_ENV === "production"
      ? "  ✔ الحسابات: مدير + أخصائي (كلمة المرور من SEED_PASSWORD)"
      : `  ✔ الحسابات: مدير + أخصائي (كلمة المرور: ${DEV_PASSWORD})`
  );
  return profile.id;
}
...
async function seedSettings() {
  const settings: Record<string, string> = {
    ...
    "site.description":
      "منصة تساعد الطالب على اكتشاف ميوله المهنية، والوصول إلى مكتبة رقمية، وحجز استشارة مع أخصائي التوجيه المهني.",
